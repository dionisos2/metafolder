...
//! restricted to entries owned exclusively by the current repository.
...
use metafolder_core::entry::{Value, ZERO_UUID};
...
pub fn execute(
    ...
    // One CTE per sort key: the entry's representative row for that field
    ...
    for (i, key) in sort.iter().enumerate() {
        ...
        ctes.push((
            format!("_s{i}"),
            format!(
                "SELECT metadata_uuid, grp, vnum, vtext, vblob FROM ( \
                   SELECT metadata_uuid, {grp} AS grp, {num} AS vnum, \
                          {text} AS vtext, {blob} AS vblob, \
                          ROW_NUMBER() OVER (PARTITION BY metadata_uuid \
        ...
        joins.push_str(&format!(" LEFT JOIN _s{i} ON _s{i}.metadata_uuid = _res.uuid"));
        select_cols.push_str(&format!(
            ", CASE WHEN _s{i}.metadata_uuid IS NULL THEN 1 ELSE 0 END AS nf{i}, \
               COALESCE(_s{i}.grp, -1) AS g{i}, COALESCE(_s{i}.vnum, {NUM_SENTINEL}) AS n{i}, \
               COALESCE(_s{i}.vtext, '') AS t{i}, COALESCE(_s{i}.vblob, x'') AS b{i}"
        ));
        // Entries without the sort field always come last, whatever `order`.
...
/// component order (5 per sort key, then the entry UUID).
...
impl<'a> Compiler<'a> {
    ...
    /// the universe: entries owned exclusively by the current repository.
    ...
    fn new(conn: &'a Connection, cache: &'a mut TreeCache, db_id: Uuid) -> Self {
        let ctes = vec![(
            "_repo".to_string(),
            "SELECT m1.metadata_uuid AS uuid FROM metadata_db m1 \
             WHERE m1.db_id = ? \
    ...
    fn compile_node(&mut self, q: &Query) -> Result<String, ApiError> {
        match q {
            Query::IsPresent { field } => {
                ...
                Ok(self.add(
                    "SELECT DISTINCT metadata_uuid AS uuid FROM field \
    ...
    fn scalar_predicate(&mut self, value: &Value, op: CmpOp) -> Result<String, ApiError> {
        ...
        match value {
            ...
            Value::ExternalRef { repo, entry } => {
                ...
                self.params.push(SqlValue::Blob(db::uuid_to_bytes(*entry)));
