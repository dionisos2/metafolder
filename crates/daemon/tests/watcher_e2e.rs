...
async fn test_watcher_tracks_create_rename_delete() {
    ...
    assert_eq!(hits[0], entry_uuid, "the entry must survive the rename");
    ...
    assert_eq!(hits[0], entry_uuid, "the entry must be preserved after deletion");
