...
async fn field_of(app: &Router, repo: &str, uuid: &str, name: &str) -> Option<Value> {
    let (status, body) =
        request(app, "GET", &format!("/repos/{repo}/metadata/{uuid}"), None).await;
...
async fn test_log_linear_and_filters() {
    ...
    // Filter by entry.
    let filtered = get_log(&app, &repo, &format!("?entry_uuid={uuid}")).await;
...
async fn test_rollback_by_id_and_redo() {
    ...
    let create_op = log["operations"]
        ...
        .unwrap()
        .iter()
        .find(|o| o["op_type"] == "create_entry" && o["entity_uuid"] == json!(uuid))
        .unwrap()["id"]
        .as_i64()
        .unwrap();
    ...
    let (_, after) = request(&app, "GET", &format!("/repos/{repo}/metadata/{uuid}"), None).await;
    ...
    let (_, redone) = request(&app, "GET", &format!("/repos/{repo}/metadata/{uuid}"), None).await;
...
#[tokio::test]
async fn test_rollback_restores_deleted_entry_with_field_ids() {
...
async fn test_rollback_to_empty_state() {
    ...
    let (_, all) = request(&app, "GET", &format!("/repos/{repo}/metadata"), None).await;
...
async fn test_prune_before_makes_weak_root() {
    ...
    let ops = log["operations"].as_array().unwrap();
    assert_eq!(ops.len(), 3);
    let target = ops[1]["id"].as_i64().unwrap(); // our create_entry
