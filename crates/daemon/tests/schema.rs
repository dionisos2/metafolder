...
async fn create(app: &Router, repo: &str, fields: Value) -> (StatusCode, Value) {
    request(app, "POST", &format!("/repos/{repo}/metadata"), Some(json!({"fields": fields}))).await
}
...
async fn test_per_type_cardinality() {
    ...
    // A film: rating is capped at one row; untyped entries are not.
    ...
    let (status, body) = request(
        &app,
        "POST",
        &format!("/repos/{repo}/metadata/{film_uuid}/fields"),
    ...
    // Untyped entry: no film constraints, two ratings allowed.
    let (_, untyped) =
    ...
    let (status, _) = request(
        &app,
        "POST",
        &format!("/repos/{repo}/metadata/{untyped_uuid}/fields"),
    ...
    let (_, film_now) =
        request(&app, "GET", &format!("/repos/{repo}/metadata/{film_uuid}"), None).await;
    ...
    let (status, body) = request(
        &app,
        "DELETE",
        &format!("/repos/{repo}/metadata/{film_uuid}/fields/{name_id}"),
        None,
    )
    .await;
    ...
    // Deleting the whole entry stays allowed.
    let (status, _) =
        request(&app, "DELETE", &format!("/repos/{repo}/metadata/{film_uuid}"), None).await;
...
async fn test_delta_validation_ignores_untouched_fields() {
    ...
    // An entry violating the rating constraint is created while the schema
