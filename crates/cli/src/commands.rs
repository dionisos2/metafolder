...
// ── Entry manipulation (spec-data-model) ──────────────────────────────────────
...
pub fn list(ctx: &Ctx, limit: Option<usize>) -> Result<i32, CliError> {
    ...
    loop {
        ...
        let resp = ctx.client.get(&format!("{base}/metadata"), &params)?;
...
pub fn get(ctx: &Ctx, target: &str, fields: Option<&[String]>) -> Result<i32, CliError> {
    let base = ctx.repo_base()?;
    let entries = match parse_target(target)? {
        Target::Entry(uuid) => {
            let mut entry =
                ctx.client.get(&format!("{base}/metadata/{}", uuid.as_simple()), &[])?;
            if let (Some(filter), Some(rows)) = (fields, entry["fields"].as_array_mut()) {
                rows.retain(|f| {
...
pub fn create(ctx: &Ctx, specs: &[String], force: bool) -> Result<i32, CliError> {
    ...
    let resp = ctx.client.post(&format!("{base}/metadata"), &body)?;
...
pub fn set(ctx: &Ctx, target: &str, spec: &str, force: bool) -> Result<i32, CliError> {
    ...
    match parse_target(target)? {
        Target::Entry(uuid) => {
            ...
            ctx.client.request(
                "PATCH",
                &format!("{base}/metadata/{}", uuid.as_simple()),
                &[],
                Some(&body),
            )?;
        }
...
pub fn add(ctx: &Ctx, uuid: &str, spec: &str, force: bool) -> Result<i32, CliError> {
    ...
    let (name, value) = parse_spec(spec)?;
    let body = json!({"name": name, "value": value, "force": force});
    ctx.client.post(&format!("{base}/metadata/{}/fields", uuid.as_simple()), &body)?;
    Ok(0)
}
...
pub fn unset(ctx: &Ctx, uuid: &str, field_id: i64, force: bool) -> Result<i32, CliError> {
    ...
    ctx.client.request(
        "DELETE",
        &format!("{base}/metadata/{}/fields/{field_id}", uuid.as_simple()),
        &[],
...
pub fn delete(ctx: &Ctx, target: &str, force: bool) -> Result<i32, CliError> {
    ...
    match parse_target(target)? {
        ...
        Target::Predicate(query) => {
            ...
            if !force && !confirm(&format!("Delete {} entries? [y/N] ", uuids.len()))? {
                eprintln!("aborted");
                return Ok(1);
            }
            for uuid in &uuids {
                ctx.client.request("DELETE", &format!("{base}/metadata/{uuid}"), &[], None)?;
            }
            println!("{}", uuids.len());
        }
    }
...
pub fn reconcile(ctx: &Ctx, entry: Option<&str>, raw_json: bool) -> Result<i32, CliError> {
    ...
    let path = match entry {
        Some(uuid) => {
            let uuid = Uuid::parse_str(uuid)
                .map_err(|_| CliError::Usage(format!("invalid entry UUID: '{uuid}'")))?;
            format!("{base}/metadata/{}/reconcile", uuid.as_simple())
        }
...
fn format_reconcile(resp: &Json) -> String {
    ...
    if !candidates.is_empty() {
        ...
        for candidate in candidates {
            out.push_str(&format!(
                "\n  {}  {}",
                candidate["entry_uuid"].as_str().unwrap_or("?"),
...
/// One line per violation: entry, activating type (`-` for global
/// constraints), field, constraint kind, message.
fn format_violation(violation: &Json) -> String {
    format!(
        "{}  {}  {}  {}  {}",
        violation["entry_uuid"].as_str().unwrap_or("?"),
...
mod tests {
    ...
    fn test_format_reconcile_with_candidates() {
        let resp = json!({
            ...
            "moved": 1,
            "candidates": [{
                "entry_uuid": "abc00000000000000000000000000001",
