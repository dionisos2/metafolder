// entry-list column specs (spec-gui "entry-list panel type"): the
...
import { parseColumns, isSortable, cellQuickText, cellText } from '../../panel-types/entry-list/columns.js';
...
describe('cellText (asynchronous display)', () => {
  const ctx = (overrides = {}) => ({
    ...
    getEntry: vi.fn(async () => entry([{ name: 'name', value: str('jazz') }])),
    ...overrides,
  });
  ...
  test('field~target follows refs and shows the target field', async () => {
    ...
    expect(c.getEntry).toHaveBeenCalledWith('1111');
  });
  ...
  test('a multi-map target field joins every row', async () => {
    ...
    const c = ctx({ getEntry: vi.fn(async () => target) });
  ...
  test('a missing target entry falls back to the raw uuid', async () => {
    const e = entry([{ name: 'tags', value: ref('1111') }]);
    const c = ctx({ getEntry: vi.fn(async () => { throw new Error('gone'); }) });
  ...
  test('a target entry without the field falls back to the raw uuid', async () => {
    const e = entry([{ name: 'tags', value: ref('1111') }]);
    const c = ctx({ getEntry: vi.fn(async () => entry([])) });
    expect(await cellText(parseColumns('tags~name')[0], e, c)).toBe('1111');
  });
  ...
  test('multi-map rows resolve independently', async () => {
    ...
    const c = ctx({
      getEntry: vi.fn(async (uuid: string) => entry([{ name: 'name', value: str(names[uuid]) }])),
    });
