...
function entries(): Metarecord<string, Entry> {
...
function setup() {
  ...
  return { resolver: createPathResolver(getMetarecord), getMetarecord, db };
}
...
describe('createPathResolver', () => {
  ...
  test('invalidate forces a re-fetch', async () => {
    const { resolver, getMetarecord, db } = setup();
    await resolver.resolveUuid('take5');
    ...
    expect(getMetarecord.mock.calls.filter(([u]) => u === 'take5').length).toBe(2);
  });

  test('entries without mfr_path reject', async () => {
    const getMetarecord = async () => ({ uuid: 'x', fields: [] });
    const resolver = createPathResolver(getMetarecord);
